...
- WP03 through WP10 must be accepted before WP11 performs the governed-document sync.
...
Document only executed commands and evidence, prove WP11's governed-doc sync is
...
2. Require committed WP11 receipt `docs/governance/p0-governed-doc-sync.json` to validate against that exact closed schema, including full 40-lowercase-hex `producer_baseline_commit` and `synchronized_baseline_commit`.
...
9. Execute the exact self-contained drift constant; require it to resolve `receipt_commit` itself and include the receipt path, and block closure on any difference.
